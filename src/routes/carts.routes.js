...
router.delete("/:cid", async (req, res) => {
    ...
    try {
        ...
        // Paso 3: Usando el Metodo emptyCart Para dejar el carrito vacio
        const result = await controller.emptyCart(cid, newContent)

        // Aca Mandamos la respuesta al cliente
        res.status(200).send({ status: 'OK. Cart Vaciado', data: result })
...
router.delete('/:cid/products/:pid', async (req, res) => {
    ...
    try {
        ...
        const cart = await controller.getCartByIdnotFormat(cid)
        ...
        const deleteProductoOncart = products.filter(item => item._id !== pid)
        ...
        // Aca Mandamos la respuesta al cliente
        res.status(200).send({ status: `OK. Product Deleted On Cart ID: ${cid}`, data: 'Estoy revisando' })
